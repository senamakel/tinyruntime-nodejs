...
use super::{DEFAULT_VERSION, NodeProvider, setup};

fn provider() -> NodeProvider {
    NodeProvider {
...
#[test]
fn the_default_version_is_a_version() {
    assert!(crate::major(DEFAULT_VERSION).is_some());
}
...
async fn the_router_can_describe_this_provider_over_a_bus() -> TinyBusResult<()> {
    ...
    assert!(descriptor.executables.contains(&"npm".to_string()));
    Ok(())
}
...
#[tokio::test]
async fn a_directory_that_is_not_a_toolchain_is_reported_empty_rather_than_failing()
-> TinyBusResult<()> {
    // The router scans a cache full of directories that are not toolchains. If
...
async fn detecting_a_host_interpreter_answers_rather_than_failing() -> TinyBusResult<()> {
    // Whether this machine has Node is not the point: the call must complete
...
#[tokio::test]
async fn a_version_that_is_not_one_is_refused_with_a_readable_reason() -> TinyBusResult<()> {
    let bus = MemoryBus::new();
    Broker::new().spawn(bus.clone());
    ...
    assert!(error.to_string().contains("latest"), "got `{error}`");
    Ok(())
}
