...
`tinyruntime-nodejs` is the Node.js **provider** for `tinyruntime`. It supplies
...
one member: `crates/tinyruntime-nodejs`, built as both an `rlib` and the
...
└── tinyruntime-nodejs/
    ...
    │   ├── version/         # what counts as a compatible Node.js version
    │   ├── system/          # finding an interpreter the host already has
    │   ├── distribution/    # which archive nodejs.org publishes for this host
    │   │   └── host.rs      # the host-triple table
    │   ├── layout/          # where an install keeps its executables
...
This repository answers five questions about Node.js and nothing else:
...
one network call it is allowed to make is reading `SHASUMS256.txt`, so the
...
- `cargo test --test harness_protocol` — run the harness suite against a real
  `node`. It skips when the machine has none.
- `cargo run -p tinyruntime-nodejs --example basic` — run the bundled example.
...
- The harness is the one part of this crate that is not Rust, so
  ...
  runs jobs through it. Any change to `pool_worker.js` needs a test there.
- Distribution selection is testable without a network. Keep the parsing and
  ranking separable from the request that fetches the index, and test it against
  a realistic index body.
...
`vX.Y.Z`, builds `crates/tinyruntime-nodejs` as a TinyBus module for every supported
